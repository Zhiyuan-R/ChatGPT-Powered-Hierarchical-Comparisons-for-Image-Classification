...
class CUBDataset(datasets.ImageFolder):
    ...
    def __init__(self,
        ...
        # img_root = os.path.join(root, 'sparrow_images')
        ...
        # path_to_index = os.path.join(root, 'sparrow_images.txt')
    ...
    def __getitem__(self, index):
        ...
        if self.transform_ is not None:
            if isinstance(self.transform_, dict):
                dict_tmp = {}
                for k, v in self.transform_.items():
                    dict_tmp[k] = v(sample)
                sample = dict_tmp
            else:
...
class color_quantized():
    ...
    def __call__(self, img):
        ...
        quantized_image = img.convert('P', palette=Image.ADAPTIVE, colors=self.k)
...
class edge_det():
    ...
    def __call__(self, img):
        ...
        result = ImageChops.blend(img, edges.convert('RGB'), self.alpha)
